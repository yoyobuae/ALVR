...
// Semver compatible versions will produce the same protocol ID. protocol ID are not ordered
// As a convention, encode/decode the protocol ID bytes as little endian.
pub fn protocol_id() -> u64 {
    let string_id = if ALVR_VERSION.pre.is_empty() {
        ALVR_VERSION.major.to_string()
    } else {
        format!(
            "{}.{}.{}-{}",
            ALVR_VERSION.major, ALVR_VERSION.minor, ALVR_VERSION.patch, ALVR_VERSION.pre
        )
    };

    hash_string(&string_id)
}
